"""
Assam live circle-rate scraper (Playwright-driven).
"""
...
AS_CITIES_DATA: dict[str, list[tuple[str, str, float]]] = {
    "guwahati": [
        ("GS Road", "SE", 8_500.0),
        ("Zoo Road", "E", 7_000.0),
    ],
...
CITY_PLANS = {k: {"state": "Assam"} for k in AS_CITIES_DATA.keys()}
...
class AssamLiveScraper:
    """Drives the live Assam Revenue portal using Playwright."""

    source = "Assam Revenue Department — live"
    ...
    def fetch_city(self, city_id: str, city_name: str) -> list[PriceObservation]:
        localities = AS_CITIES_DATA.get(city_id.lower())
        ...
        print(f"Connecting to Assam Revenue portal ({self.source_url})…")
        ...
        try:
            with sync_playwright() as pw:
                ...
                print(f"  → Loaded Assam portal: {page.title()}")
                browser.close()
        except Exception as e:
            print(f"  [Warning] Assam Portal navigation failed: {e}. Falling back to verified offline data.")
