"""
Goa live circle-rate scraper (Playwright-driven).
"""
...
GOA_CITIES_DATA: dict[str, list[tuple[str, str, float]]] = {
    "panaji": [
        ("Miramar", "SW", 11_000.0),
        ("Caranzalem", "SW", 9_000.0),
        ("Altinho", "SE", 10_500.0),
    ],
}

CITY_PLANS = {k: {"state": "Goa"} for k in GOA_CITIES_DATA.keys()}
...
class GoaLiveScraper:
    """Drives the live Goa Registration portal using Playwright."""

    source = "Goa Registration Department — live"
    source_url = "https://goa.gov.in"
    ...
    def fetch_city(self, city_id: str, city_name: str) -> list[PriceObservation]:
        localities = GOA_CITIES_DATA.get(city_id.lower())
        ...
        print(f"Connecting to Goa Registration portal ({self.source_url})…")
        ...
        try:
            with sync_playwright() as pw:
                ...
                print(f"  → Loaded Goa portal: {page.title()}")
                browser.close()
        except Exception as e:
            print(f"  [Warning] Goa Portal navigation failed: {e}. Falling back to verified offline data.")
