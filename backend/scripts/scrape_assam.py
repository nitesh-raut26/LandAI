#!/usr/bin/env python3
"""
Scrape live Assam circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.assam_live import (  # noqa: E402
    CITY_PLANS, AssamLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = AssamLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Assam Circle Rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Assam Revenue & Disaster Management Department — Circle Rates live scrape (FY {date.today().year})",
        "source_url": "https://assam.gov.in",
        "source_document": "Circle Rates",
        "license": "GODL-India",
        "state": "Assam",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Assam Circle Rates",
        "official": True,
    }, indent=2))
