#!/usr/bin/env python3
"""
Scrape live Bihar circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.bihar_live import (  # noqa: E402
    CITY_PLANS, BiharLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = BiharLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Bihar MVR)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Bihar Registration Department — MVR live scrape (FY {date.today().year})",
        "source_url": "https://registration.bihar.gov.in",
        "source_document": "Minimum Value Register — open-land rate",
        "license": "GODL-India",
        "state": "Bihar",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Bihar MVR",
        "official": True,
    }, indent=2))
