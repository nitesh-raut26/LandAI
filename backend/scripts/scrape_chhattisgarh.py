#!/usr/bin/env python3
"""
Scrape live Chhattisgarh circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.chhattisgarh_live import (  # noqa: E402
    CITY_PLANS, ChhattisgarhLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = ChhattisgarhLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Chhattisgarh Market Values)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Chhattisgarh Registration & Stamps Department — Market Value live scrape (FY {date.today().year})",
        "source_url": "https://cg.nic.in",
        "source_document": "Market Value",
        "license": "GODL-India",
        "state": "Chhattisgarh",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Chhattisgarh Market Values",
        "official": True,
    }, indent=2))
