#!/usr/bin/env python3
"""
Scrape live Delhi circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.delhi_live import (  # noqa: E402
    CITY_PLANS, DelhiLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = DelhiLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Delhi rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Delhi Revenue Department live scrape (FY {date.today().year})",
        "source_url": "https://revenue.delhi.gov.in",
        "source_document": "Delhi circle rates — open-land rate",
        "license": "GODL-India",
        "state": "Delhi",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Delhi rates",
        "official": True,
    }, indent=2))
