#!/usr/bin/env python3
"""
Scrape live Goa circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.goa_live import (  # noqa: E402
    CITY_PLANS, GoaLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = GoaLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Goa Circle Rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Goa Registration Department — Circle Rates live scrape (FY {date.today().year})",
        "source_url": "https://goa.gov.in",
        "source_document": "Circle Rates",
        "license": "GODL-India",
        "state": "Goa",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Goa Circle Rates",
        "official": True,
    }, indent=2))
