#!/usr/bin/env python3
"""
Scrape live Haryana circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.haryana_live import (  # noqa: E402
    CITY_PLANS, HaryanaLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = HaryanaLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Haryana rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Haryana Jamabandi live scrape (FY {date.today().year})",
        "source_url": "https://jamabandi.nic.in",
        "source_document": "Haryana collector rates — open-land rate",
        "license": "GODL-India",
        "state": "Haryana",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Haryana rates",
        "official": True,
    }, indent=2))
