#!/usr/bin/env python3
"""
Scrape live Himachal Pradesh circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.himachal_live import (  # noqa: E402
    CITY_PLANS, HimachalLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = HimachalLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Himachal Circle Rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Himachal Pradesh Revenue Department — Circle Rates live scrape (FY {date.today().year})",
        "source_url": "https://himachal.nic.in",
        "source_document": "Circle Rates",
        "license": "GODL-India",
        "state": "Himachal Pradesh",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Himachal Circle Rates",
        "official": True,
    }, indent=2))
