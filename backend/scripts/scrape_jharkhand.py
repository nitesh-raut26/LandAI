#!/usr/bin/env python3
"""
Scrape live Jharkhand circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.jharkhand_live import (  # noqa: E402
    CITY_PLANS, JharkhandLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = JharkhandLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Jharkhand rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Jharkhand Revenue Department live scrape (FY {date.today().year})",
        "source_url": "https://regd.jharkhand.gov.in",
        "source_document": "Jharkhand circle rates — open-land rate",
        "license": "GODL-India",
        "state": "Jharkhand",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Jharkhand rates",
        "official": True,
    }, indent=2))
