#!/usr/bin/env python3
"""
Scrape live Karnataka circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.karnataka_live import (  # noqa: E402
    CITY_PLANS, KarnatakaLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = KarnatakaLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Karnataka Kaveri)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Karnataka Kaveri Online Services — Guidance Value live scrape (FY {date.today().year})",
        "source_url": "https://kaverionline.karnataka.gov.in",
        "source_document": "Guideline Value",
        "license": "GODL-India",
        "state": "Karnataka",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Karnataka Kaveri Guidance Values",
        "official": True,
    }, indent=2))
