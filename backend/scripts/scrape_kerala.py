#!/usr/bin/env python3
"""
Scrape live Kerala circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.kerala_live import (  # noqa: E402
    CITY_PLANS, KeralaLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = KeralaLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Kerala Fair Value)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Kerala Registration Department — Fair Value live scrape (FY {date.today().year})",
        "source_url": "https://kerala.gov.in",
        "source_document": "Fair Value of Land",
        "license": "GODL-India",
        "state": "Kerala",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Kerala Fair Value",
        "official": True,
    }, indent=2))
