#!/usr/bin/env python3
"""
Scrape live Madhya Pradesh circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.madhya_pradesh_live import (  # noqa: E402
    CITY_PLANS, MadhyaPradeshLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = MadhyaPradeshLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (MP Collector Rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"MP Inspector General of Registration — Collector Rates live scrape (FY {date.today().year})",
        "source_url": "https://mpigr.gov.in",
        "source_document": "Collector Rates",
        "license": "GODL-India",
        "state": "Madhya Pradesh",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official MP Collector Rates",
        "official": True,
    }, indent=2))
