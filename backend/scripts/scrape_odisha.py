#!/usr/bin/env python3
"""
Scrape live Odisha circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.odisha_live import (  # noqa: E402
    CITY_PLANS, OdishaLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = OdishaLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Odisha Benchmark)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Odisha Inspector General of Registration — Benchmark Valuation live scrape (FY {date.today().year})",
        "source_url": "https://odisha.gov.in",
        "source_document": "Benchmark Valuation",
        "license": "GODL-India",
        "state": "Odisha",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Odisha Benchmark Valuation",
        "official": True,
    }, indent=2))
