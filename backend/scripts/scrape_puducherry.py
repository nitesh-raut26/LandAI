#!/usr/bin/env python3
"""
Scrape live Puducherry circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.puducherry_live import (  # noqa: E402
    CITY_PLANS, PuducherryLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = PuducherryLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Puducherry Guideline)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Puducherry Registration Department — Guideline live scrape (FY {date.today().year})",
        "source_url": "https://puducherry.gov.in",
        "source_document": "Guideline Value",
        "license": "GODL-India",
        "state": "Puducherry",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Puducherry Guideline Values",
        "official": True,
    }, indent=2))
