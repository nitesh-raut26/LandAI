#!/usr/bin/env python3
"""
Scrape live Rajasthan circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.rajasthan_live import (  # noqa: E402
    CITY_PLANS, RajasthanLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = RajasthanLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Rajasthan DLC)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Rajasthan Registration & Stamps Department — DLC live scrape (FY {date.today().year})",
        "source_url": "https://igrs.rajasthan.gov.in",
        "source_document": "DLC Rates",
        "license": "GODL-India",
        "state": "Rajasthan",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Rajasthan DLC Rates",
        "official": True,
    }, indent=2))
