#!/usr/bin/env python3
"""
Scrape live Telangana circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.telangana_live import (  # noqa: E402
    CITY_PLANS, TelanganaLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = TelanganaLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Telangana Dharani)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Telangana IGRS — Dharani Guidance Values live scrape (FY {date.today().year})",
        "source_url": "https://registration.telangana.gov.in",
        "source_document": "Guideline Value",
        "license": "GODL-India",
        "state": "Telangana",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Telangana Dharani Guidance Values",
        "official": True,
    }, indent=2))
