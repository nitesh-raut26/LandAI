#!/usr/bin/env python3
"""
Scrape live Uttar Pradesh circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.up_live import (  # noqa: E402
    CITY_PLANS, UPLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = UPLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (UP rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Uttar Pradesh Registration Department live scrape (FY {date.today().year})",
        "source_url": "https://igrsup.gov.in",
        "source_document": "UP circle rates — open-land rate",
        "license": "GODL-India",
        "state": "Uttar Pradesh",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official UP rates",
        "official": True,
    }, indent=2))
