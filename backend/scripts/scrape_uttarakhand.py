#!/usr/bin/env python3
"""
Scrape live Uttarakhand circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.uttarakhand_live import (  # noqa: E402
    CITY_PLANS, UttarakhandLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = UttarakhandLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (Uttarakhand Circle Rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"Uttarakhand Stamp & Registration Department — Circle Rates live scrape (FY {date.today().year})",
        "source_url": "https://registration.uk.gov.in",
        "source_document": "Circle Rates",
        "license": "GODL-India",
        "state": "Uttarakhand",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official Uttarakhand Circle Rates",
        "official": True,
    }, indent=2))
