#!/usr/bin/env python3
"""
Scrape live West Bengal circle rates and write a VERIFIED artifact.
"""
...
from app.ingestion.scrapers.circle_rates.west_bengal_live import (  # noqa: E402
    CITY_PLANS, WestBengalLiveScraper, available,
)
...
def main(city_ids: list[str]) -> int:
    ...
    scraper = WestBengalLiveScraper(throttle_s=1.0)

    rows = []
    for cid in city_ids:
        name = cid.replace("_", " ").title()
        print(f"Scraping {cid} (West Bengal rates)…")
    ...
    (SOURCES / f"{SOURCE_KEY}.meta.json").write_text(json.dumps({
        "source": f"West Bengal Directorate of Registration live scrape (FY {date.today().year})",
        "source_url": "https://wbregistration.gov.in",
        "source_document": "West Bengal circle rates — open-land rate",
        "license": "GODL-India",
        "state": "West Bengal",
        "retrieved_at": datetime.now(timezone.utc).date().isoformat(),
        "method": "live scrape/extraction of official West Bengal rates",
        "official": True,
    }, indent=2))
